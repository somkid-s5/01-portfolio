                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                            ...
                            <section>
                                ...
                                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {project.key_features.map((feature: string, index: number) => (
                                        <li key={index} className="flex items-start gap-3 bg-gray-900/20 p-4 rounded border border-gray-800/50">
                    ...
                    <div className="space-y-8">
                        <div className="bg-[#0a0a0a] border border-gray-800 rounded-xl p-6 space-y-6 sticky top-32">
                            <div>
                                <h3 className="text-sm font-mono text-gray-500 mb-4 uppercase tracking-wider">Project Links</h3>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {project.tech_stack?.map((tech: string) => (
                                        <span key={tech} className="px-3 py-1 text-xs font-mono bg-gray-900 text-emerald-400 border border-emerald-500/20 rounded">
                                            {tech}
                                        </span>
                                    ))}
                                </div>
                                ...
                                <div className="space-y-2 text-sm font-mono">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-gray-300">
                                            {project.started_at ? new Date(project.started_at).toLocaleDateString() : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Finished:</span>
                                        <span className="text-gray-300">
                                            {project.finished_at ? new Date(project.finished_at).toLocaleDateString() : 'In Progress'}
                                        </span>
                                    </div>
                                </div>
                            </div>
